...
public class Application extends JFrame {
    ...
    public void addQuery(Query query) {
        ...
        // Connect the query to the twitter source (add as observer)
    ...
    public Application() {
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                List<MapMarker> markersCovering = getMarkersCovering(pos, pixelWidth(p));
                for (MapMarker m : markersCovering) {
                    // typecast the marker into the preferred subclass, so we can use the getters
                    MapMarkerPrettier marker = (MapMarkerPrettier) m;
                    // use the HTML abilities of ToolTip to show the Twitter profile image and status
                    map().setToolTipText("<html><body \"><img src=\"" + marker.getProfileImgURL() + "\" width=48 height=48 />" + marker.getStatusText() + "</html>");
                }
            }
        });
    }
    ...
    public void terminateQuery(Query query) {
        // Disconnect the expiring query from the twitter source (remove as observer)
